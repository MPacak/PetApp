 ...
 1,
                        ...
                        object : Geocoder.GeocodeListener{
                            override fun onGeocode(addresses: MutableList<Address>) {
                                if (addresses.isNotEmpty()) {
                                    ...
                                    Log.d("OrgPagerAdapter", "entering return to main thread'")
                                    map.clear()
                                    map.addMarker(
                                        MarkerOptions()
                                            .title(name)
                                    ...
                                    Log.w("OrgPagerAdapter", "New geocoding returned no results for '$fullAddress'")
                                }
                    ...
                    Executors.newSingleThreadExecutor().execute {
                        val addresses: List<Address>? = try {
                            geocoder.getFromLocationName(fullAddress, */
/* maxResults *//*
 1)
                        } catch (e: IOException) {
                            null
...
/** Launch an Intent to open the full address in the Google Maps app *//*
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        lifecycleOwner.lifecycle.addObserver(LifecycleEventObserver { _, event ->
            Log.w("OrgPagerAdapter", "Bind view holder")
